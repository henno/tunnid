...
// REMOVE FILTERS
    ...
    function filter() {
        ...
        $('.lessonrow:visible').each(function (index) {
            $(this).find('.count').html((index + 1) * 2);
        });
        ...
        // Count lessons
        let countOfLessons = 1;
        $($('.lessonrow:visible').get().reverse()).prevAll().filter(":visible").each(function (i, e) {
            if ($(e).hasClass('daterow')) {
                $(e).find('.countOfLessons').text(countOfLessons *2)
                countOfLessons = 0;
            } else {
                countOfLessons++
            }
